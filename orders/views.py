...
from customization.models import ProductCustomization
@login_required
def checkout(request):
    ...
    if request.method == "POST":
        ...
        customization_id = request.session.get("customization_id")

        if customization_id:

            try:

                customization = ProductCustomization.objects.get(
                ...
                del request.session["customization_id"]
...
def payment(request, order_id):
    ...
    if request.method == "POST":
        ...
        order.transaction_id = request.POST.get(
            "transaction_id",
            ""
        )
...
def download_invoice(request, order_id):
    ...
    pdf.drawString(160, 800, "DIGITAL GALLERIA")
    ...
    pdf.drawString(
        50,
        y,
        f"Grand Total : ₹{order.total_price}"
    )
    ...
    pdf.drawString(
        110,
        y,
        "Thank You For Shopping With Digital Galleria!"
    )
